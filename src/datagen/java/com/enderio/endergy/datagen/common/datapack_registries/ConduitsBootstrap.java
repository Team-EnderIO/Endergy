...
public class ConduitsBootstrap {
    public static void bootstrap(BootstrapContext<Conduit<?, ?>> context) {
        ...
        register(context, EndergyConduits.CRUDE_ENERGY, (desc) -> new EnergyConduit(EnderIOEndergy.id("block/conduit/crude_energy"), desc, 20));
        register(context, EndergyConduits.COPPER_ENERGY, (desc) -> new EnergyConduit(EnderIOEndergy.id("block/conduit/copper_energy"), desc, 40));
        register(context, EndergyConduits.IRON_ENERGY, (desc) -> new EnergyConduit(EnderIOEndergy.id("block/conduit/iron_energy"), desc, 80));
        register(context, EndergyConduits.GOLD_ENERGY, (desc) -> new EnergyConduit(EnderIOEndergy.id("block/conduit/gold_energy"), desc, 160));
